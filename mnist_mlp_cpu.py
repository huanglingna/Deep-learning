...
class MNIST_MLP(object):
    def __init__(self, batch_size=100, input_size=784,hidden1=256,hidden2=128,out_classes=10,lr=0.01, max_epoch=50,print_iter=100):
    ...
    def load_mnist(self,file_dir, is_images = 'True'):
        ...
        if is_images:
            mat_data = mat_data.astype(np.float32) / 255.0  # 归一化到0-1，消除数值范围影响
            mat_data = mat_data - np.mean(mat_data, axis=0)  # 中心化，梯度更新更平滑
    ...
    def init_model(self):
        ...
        for layer in self.update_layer_list:
            std = np.sqrt(2.0 / (layer.num_input + layer.num_output))
            layer.init_param(std=std)  # 替换原固定0.01的标准差
